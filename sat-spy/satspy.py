...
class SatSpy:
    ...
    def get_max_resolution(self) -> None:
        ...
        if "max_resolution" in locals() and max_resolution > self.resolution:
            ...
            if self.verbose:
                print(
                    "The resolution entered is too high for a Direct Download (DD) and has been set to the maximum resolution achievable withis method. Consider using the Split and Merge Download (SMD) to always attain the highest resolution independently of the area."
                )
