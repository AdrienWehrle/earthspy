...
class satspy:
    ...
    def configure_connection(self) -> SHConfig:
    ...
    def set_query_parameters(
        ...
        if max_resolution == self.data_collection_resolution and self.download_method == 'SPD':
            self.download_method = 'DD'

            if self.verbose:
                print("Split and Merge Download (SPD) is not needed to reach the maximum data resolution. Switching to Direct Download (DD).")
    ...
    def get_data_collection(self):
        ...
        self.data_collection = DataCollection[self.data_collection_str]
    ...
    def get_data_collection_resolution(self):
        ...
        if satellite == 'SENTINEL1':
            ...
            if self.verbose:
                print("Satellite resolution not implemented yet. Data collection resolution is set to 1km and will be refined.")
    ...
    def convert_bounding_box_coordinates(self):
        ...
        return self.bouding_box_meters
    ...
    def get_max_resolution(self) -> None:
        ...
        try:
            ...
            raise IndexError(
                f"Calculated resolution above 10km forced by {origin} dimension(s), please narrow down your study area."
            )
        ...
        if 'max_resolution' in locals() and max_resolution > self.resolution:
            ...
            if self.verbose:
                print("The resolution entered is too high for a Direct Download (DD) and has been set to the maximum resolution achievable withis method. Consider using the Split and Merge Download (SMD) to always attain the highest resolution independently of the area.")
    ...
    def get_split_boxes(self):
        ...
        if self.verbose:
            print(f"Initial bounding box will be split into a ({nb_boxes_x}, {nb_boxes_y}) grid")
        ...
        bb_splitter = BBoxSplitter([bbox_polygon], "epsg:4326", (nb_boxes_x, nb_boxes_y))
        ...
        if not evaluation_script:
            ...
            if self.data_collection_str == "SENTINEL2_L2A":
                self.get_evaluation_script_from_link(
                    "https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel-2/true_color/script.js"
                )
            elif self.data_collection_str == "S1":
                self.get_evaluation_script_from_link(
                    "https://custom-scripts.sentinel-hub.com/custom-scripts/sentinel-1/sar_rvi_temporal_analysis/script.js"
                )
    ...
    def sentinelhub_request(
        ...
        loc_bbox = BBox(bbox=self.bounding_box, crs=CRS.WGS84)
        loc_size = bbox_to_dimensions(loc_bbox, resolution=self.resolution)
    ...
    def rename_output_files(self) -> None:
        ...
        for folder in folders:
            ...
            # TODO: create folder with area name if there is one
